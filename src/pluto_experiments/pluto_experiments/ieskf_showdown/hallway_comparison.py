...
def run_ieskf(map_pts, gt_poses, imu_seq, lidar_seq):
    ...
    for (t, omega_m, ax_m) in imu_seq:
        ...
        if lidar_seq and abs(lidar_seq[0][0] - t) < DT_IMU / 2:
            ...
            def z_fn(pj, _):
                z, _, ok = sm.compute_residuals_and_jacobians(scan, pj)
                return z if ok else np.array([])

            def H_fn(pj, _):
                _, H3, ok = sm.compute_residuals_and_jacobians(scan, pj)
                if not ok: return np.zeros((0, 6))
                m = H3.shape[0]
                H6 = np.zeros((m, 6)); H6[:, :3] = H3
                return H6

            z_p = z_fn(kf.pose, kf.bias)
            if z_p.size > 0:
                V = np.eye(len(z_p)) * SIGMA_SCAN**2
                kf.update(z_fn, H_fn, V, max_iter=5)
